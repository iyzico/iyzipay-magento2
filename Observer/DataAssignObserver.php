...
class DataAssignObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer): void
    {
        ...
        if (isset($installmentCount) && $installmentCount > 1 && $paymentMethod == 'iyzipay') {
            ...
            $grandTotal = !empty($grandTotal) ? $grandTotal : 0.00;
            $subTotalWithFee = !empty($subTotalWithFee) ? $subTotalWithFee : 0.00;
            $currency = !empty($currency) ? $currency : 'N/A';

            // Grand Total (Purchased) should include installment fee
            $grandTotalWithInstallmentFee = $grandTotal + $getInstallmentFee;

            $order->setBaseTotalPaid($grandTotalWithInstallmentFee);
            $order->setTotalPaid($grandTotalWithInstallmentFee);
            ...
            $payment->setBaseAmountPaid($grandTotalWithInstallmentFee);
            $payment->setAmountPaid($grandTotalWithInstallmentFee);
