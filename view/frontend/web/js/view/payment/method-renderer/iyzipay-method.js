define([
    ...
    return Component.extend({
        ...
        loadPaymentForm: function () {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success) {
                        // Store payment URL and token for redirect mode (quote-first approach)
        ...
        showIframeCheckoutForm: function (checkoutFormContent) {
            ...
            // Check if #iyzipay-payment exists
            var paymentElement = $('#iyzipay-payment');
            ...
            if (paymentElement.length > 0) {
                paymentElement.after(container);
            } else {
                // Fallback: append to payment method container
                $('.payment-method._active').append(container);
            }
        ...
        payWithIyzico: function () {
            ...
            // If payment URL already exists (from loadPaymentForm), use it directly (quote-first approach)
            // This prevents creating duplicate tokens
            if (self.paymentUrl()) {
                console.log("Using existing payment URL (quote-first approach)");
                window.location.href = self.paymentUrl();
                return;
            }

            // Fallback: If payment URL doesn't exist, create new token
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.success) {
                        // Store payment URL for future use
                        self.paymentUrl(response.url);
                        if (response.token) {
                            self.paymentToken(response.token);
                        }
