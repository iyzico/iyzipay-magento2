...
class Webhook implements WebhookInterface
{
    ...
    public function processWebhook(WebhookData $webhookData): void
    {
        try {
            ...
            // Find order_id by token (may be null in quote-first approach)
            ...
            if ($orderId) {
                ...
                // Order doesn't exist yet, create it from quote (quote-first approach)
                ...
                if ($quoteId) {
                    try {
                        ...
                        // Check if payment is successful before creating order
                        $paymentStatus = $response->getPaymentStatus();
                        $status = $response->getStatus();
                        
                        if ($status === 'success' && $paymentStatus !== 'FAILURE') {
                            // Create order from quote
                            $newOrderId = $this->orderService->createOrderFromQuote(
                                $quoteId,
                                null, // No customer session in webhook context
                                $this->cartManagement,
                                $response
                            );
                            
                            // Assign order ID to iyzico order job table
                            $this->orderJobService->assignOrderIdToIyzicoOrderJob(
                                (string)$newOrderId,
                                (string)$quoteId
                            );
                        } else {
                            $this->iyziWebhookLogger->info(
                                sprintf(
                                    'Payment not successful, order not created. Status: %s, PaymentStatus: %s',
                                    $status,
                                    $paymentStatus
                                )
                            );
                        ...
                        $this->iyziWebhookLogger->error(
                            sprintf('Quote not found for quote_id: %s', $quoteId)
                        );
                    } catch (Exception $e) {
                        $this->iyziWebhookLogger->error(
                            sprintf('Error creating order from quote: %s', $e->getMessage())
                        );
                    }
                } else {
                    $this->iyziWebhookLogger->error(
                        sprintf('Quote ID not found for token: %s', $token)
                    );
                }
            }
        } catch (Exception $e) {
    ...
    public function processWebhookV3(WebhookData $webhookData): void
    {
        try {
            ...
            // Order doesn't exist yet, try to create it from quote (quote-first approach)
            $quoteId = $this->orderJobService->findQuoteIdByToken($token);
            
            if ($quoteId) {
                try {
                    // Retrieve checkout form to get payment status
                    $response = $this->orderService->retrieveAndValidateCheckoutForm($token, $conversationId);
                    ...
                    if ($status === 'success' && $paymentStatus !== 'FAILURE') {
                        ...
                        $this->orderService->updateOrderPaymentStatus((string)$newOrderId, $webhookData, 'v3');
                    } else {
                        $this->iyziWebhookLogger->info(
                            sprintf(
                                'Payment not successful, order not created. Status: %s, PaymentStatus: %s',
                ...
                $this->iyziWebhookLogger->error(
                    sprintf('Payment record not found for payment ID: %s and conversation ID: %s',
