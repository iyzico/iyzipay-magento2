...
class IyzipayRequest implements ActionInterface
{
    ...
    public function execute(): Json
    {
        try {
            ...
            // If response failed, return error immediately
            if ($responseStatus !== 'success') {
                $errorMessage = $response->getErrorMessage() ?: 'Payment initialization failed';
            ...
            if ($responseSignature === $calculateSignature) {
                ...
                // Save token and quote information for quote-first payment flow
                $this->orderJobService->saveIyziOrderJobTableForQuote(
                    $responseToken,
                    $responseConversationId,
                    $basketId
                );
